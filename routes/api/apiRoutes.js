        ...
        prices.getBin("EOS", function(EOS) {
          ...
          prices.getBin("NEO", function(NEO) {
            ...
            prices.getBin("XRP", function(XRP) {
              ...
              res.json({
                btc: Number(btc.BTCUSDT).toFixed(4),
                eth: Number(eth.ETHUSDT).toFixed(4),
                eos: Number(EOS.EOSUSDT).toFixed(4),
                neo: Number(NEO.NEOUSDT).toFixed(4),
                xrp: Number(XRP.XRPUSDT).toFixed(4)
              });
            });
          });
        });
        ...
        prices.getCob("EOS", function(EOS_cob) {
          ...
          prices.getCob("NEO", function(NEO_cob) {
            ...
            prices.getCob("XRP", function(XRP_cob) {
              ...
              res.json({
                btc: Number(BTC_cob).toFixed(4),
                eth: Number(ETH_cob).toFixed(4),
                eos: Number(EOS_cob).toFixed(4),
                neo: Number(NEO_cob).toFixed(4),
                xrp: Number(XRP_cob).toFixed(4)
              });
            });
          });
        });
